...
(** DFA parsing code, weirdly factored for convenience! **)

module type DFA_Types =
  sig
    type symbol = char;;
    type state = int;;
    type dfa = { states : int;
...
module Parser (D: DFA_Types) =
  ...
  let rec read_transitions () =
    try
      ...
      let (st, sy, st') =
        try Scanf.sscanf line "%d %c %d" (fun a b c -> (a, b, c))
        with _ -> raise (ParseError ("invalid transition: " ^ line))
...
let parse_file f =
  ...
  let (dfa, input) = parse_input chan in
